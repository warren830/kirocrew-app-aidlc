...
import {
  ...
  latestMainWorkflowStageRunFloorFromRows,
...
/**
 * A plan's task boxes record execution progress, not a change to approved work.
 * Use real Markdown task nodes so literal examples in fences, comments, HTML
 * and indented code remain byte-bound. Preserve all other bytes.
 */
function planWithoutCompletionMarks(plan: string): string {
  const task = /^([ \t]*(?:[-+*]|\d+[.)])[ \t]+\[)[xX](\](?=[ \t]|$))/;
  const positions: number[] = [];
  let offset = 0;
  for (const [index, line] of plan.split(/(\r\n|\r|\n)/).entries()) {
    if (index % 2 === 0) {
      const bom = index === 0 && line.startsWith("\uFEFF") ? 1 : 0;
      const match = task.exec(line.slice(bom));
      if (match !== null) positions.push(offset + bom + match[1].length);
    }
    offset += line.length;
  }
  if (positions.length === 0 || typeof Bun.markdown?.react !== "function") return plan;

  type TaskView = { shape: string; tasks: Array<{ checked: boolean; review: boolean }> };
  const parse = (text: string): TaskView => {
    const shape: string[] = [];
    ...
    const headingText = (value: unknown): string => {
      ...
      return node.type === "html" ? "" : headingText(node.props?.children);
    };
    const visit = (value: unknown): void => {
      if (Array.isArray(value)) { value.forEach(visit); return; }
      if (value === null || typeof value !== "object") return;
      const node = value as { type?: unknown; props?: { children?: unknown; checked?: unknown } };
      const type = typeof node.type === "string" ? node.type : "fragment";
      if (["blockquote", "html", "pre", "code"].includes(type)) {
        shape.push(`${type}:opaque`);
        return;
      }
      if (type === "h1" || type === "h2") {
        review = type === "h2" && headingText(node.props?.children).trim() === "Review";
        shape.push(`${type}:${review ? "review" : "other"}`);
        return;
      }
      shape.push(`(${type}`);
      if (type === "li" && typeof node.props?.checked === "boolean") {
        tasks.push({ checked: node.props.checked, review });
        shape.push("task");
      }
      visit(node.props?.children);
      shape.push(")");
    };
    visit(Bun.markdown.react(text));
    return { shape: shape.join("|"), tasks };
  };
  const replace = (mark: (index: number) => string): string => {
    const parts: string[] = [];
    let start = 0;
    positions.forEach((position, index) => {
      parts.push(plan.slice(start, position), mark(index));
      start = position + 1;
    });
    parts.push(plan.slice(start));
    return parts.join("");
  };
  try {
    const baseline = parse(plan);
    if (baseline.tasks.length === 0) return plan;
    const ids = baseline.tasks.map(() => 0);
    // Binary-label candidate status characters using only valid checkbox
    // states. No text is inserted, so fences/HTML/list structure cannot be
    // changed by a sentinel. Reserved all-zero/all-one codes identify tasks
    // whose source checkbox was not a candidate. This needs O(log n) parses.
    const bits = Math.ceil(Math.log2(positions.length + 2));
    for (let bit = 0; bit < bits; bit++) {
      const probe = parse(replace((index) =>
        Math.floor((index + 1) / 2 ** bit) % 2 ? "x" : " "));
      if (probe.shape !== baseline.shape || probe.tasks.length !== baseline.tasks.length) return plan;
      probe.tasks.forEach((item, index) => {
        if (item.review !== baseline.tasks[index].review) throw new Error("task scope changed");
        if (item.checked) ids[index] += 2 ** bit;
      });
    }
    const mutable = new Set<number>();
    const seen = new Set<number>();
    ids.forEach((id, index) => {
      if (id < 1 || id > positions.length) return;
      if (seen.has(id)) throw new Error("ambiguous task source");
      seen.add(id);
      if (baseline.tasks[index].checked && !baseline.tasks[index].review) mutable.add(id - 1);
    });
    return replace((index) => mutable.has(index) ? " " : plan[positions[index]]);
  } catch {
    return plan;
  }
}

function rawApprovalFingerprint(
  plan: string,
  instructions: string,
  contractHash: string,
  authority: Pick<
...
export function approvalFingerprint(
  ...
  return rawApprovalFingerprint(
    planWithoutCompletionMarks(plan), instructions, contractHash, authority,
  );
}
...
function codeGenerationApprovalArtifacts(
  ...
  const recordedFingerprint = questionsFileApprovalFingerprint(questions);
  let expectedFingerprint =
    planExists && instructionsExist && contractValid && current
      ...
      : null;
  // Unchanged approvals created before progress normalization still bind their
  // exact original bytes. Never infer a legacy checkbox pattern from a digest.
  if (expectedFingerprint !== null && current !== null) {
    const legacy = rawApprovalFingerprint(
      plan, instructions, current.contract_sha256, authority,
    );
    if (recordedFingerprint === legacy) expectedFingerprint = legacy;
  }
  return {
    plan,
    instructions,
...
/**
 * A reviewer may append evidence after generation without changing the plan
 * the human approved. A scoped request supplies only a candidate byte boundary;
 * the existing protected receipt must still commit to that exact prefix.
 * This path is never used when issuing a fresh Plan Approval question.
 */
function recordedApprovalBeforeReview(
  projectDir: string,
  authority: CodeGenerationAuthority,
  artifacts: ReturnType<typeof codeGenerationApprovalArtifacts>,
): string | null {
  try {
    const recorded = artifacts.recordedFingerprint;
    if (recorded === null || artifacts.contractHash === null || !artifacts.contractValid) return null;
    const receipt = readPlanApprovalReceipt(projectDir, authority);
    const questionsFile = toPosix(relative(projectDir, artifacts.questionsPath));
    const questionsSha256 = createHash("sha256").update(artifacts.questions, "utf-8").digest("hex");
    const promptSha256 = createHash("sha256").update(
      `${artifacts.questions.replace(/^\[Answer\]:[ \t]*.*$/gm, "[Answer]:").trimEnd()}\n`,
      "utf-8",
    ).digest("hex");
    const identity: PlanApprovalRuntimeIdentity = {
      targetId: authority.targetId, intentId: authority.intentId,
      directiveEpoch: authority.directiveEpoch, runFloor: authority.runFloor,
      fingerprint: recorded, questionsFile, promptSha256,
      sourceFloor: authority.sourceFloor, markerRevision: authority.markerRevision,
    };
    if (
      receipt === null || receipt.status !== "generation" || receipt.choice !== "Approve Plan" ||
      !runtimeIdentityMatches(receipt, identity) || receipt.questionsSha256 !== questionsSha256 ||
      receipt.certifiedSourceSha256 !== authority.sourceFloor
    ) return null;

    const stage = findStageBySlug("code-generation");
    if (stage?.review_artifact !== "code-generation-plan" || !stage.reviewer) return null;
    const body = Buffer.from(artifacts.plan, "utf-8");
    const terminal = existingReviewAppendixOffset(body);
    if (terminal === null) return null;
    const unreadable: string[] = [];
    const rows = readAuditShardEvents(projectDir, undefined, undefined, unreadable);
    if (unreadable.length > 0 || authority.runFloor === "unstarted#0") return null;
    const unitMajor = getField(readFileSync(stateFilePath(projectDir), "utf-8"), "Construction Iteration")
      ?.trim() === "unit-major";
    const mainScope = (block: string): boolean => {
      const workflow = auditBlockField(block, "Workflow");
      return (!workflow || workflow === "main") &&
        auditBlockField(block, "Stage") === "code-generation" &&
        (auditBlockField(block, "Unit")?.trim() ?? "") === (authority.unit ?? "");
    };
    const grants = rows.filter((row) =>
      row.event === "PLAN_APPROVAL_RECORDED" && mainScope(row.block) &&
      auditBlockField(row.block, "Details") === "Approve Plan" &&
      auditBlockField(row.block, "Plan Target") === authority.targetId &&
      auditBlockField(row.block, "Intent") === authority.intentId &&
      auditBlockField(row.block, "Directive Epoch") === authority.directiveEpoch &&
      auditBlockField(row.block, "Run floor") === authority.runFloor &&
      auditBlockField(row.block, "Approval Fingerprint") === recorded &&
      auditBlockField(row.block, "Questions File") === questionsFile &&
      auditBlockField(row.block, "Questions SHA-256") === questionsSha256 &&
      auditBlockField(row.block, "Prompt SHA-256") === promptSha256 &&
      auditBlockField(row.block, "Session") === receipt.session
    );
    if (grants.length === 0) return null;
    const logicalPath = toPosix(relative(docsRoot(projectDir), join(authority.stageDir, "code-generation-plan.md")));
    const decoder = new TextDecoder("utf-8", { fatal: true, ignoreBOM: true });
    for (const row of rows) {
      if (
        row.event !== "REVIEW_REQUESTED" || !mainScope(row.block) ||
        auditBlockField(row.block, "Reviewer") !== stage.reviewer ||
        !grants.some((grant) => grant.shard === row.shard
          ? grant.pos < row.pos
          : grant.timestamp < row.timestamp)
      ) continue;
      const binding = reviewRequestBindingFromBlock(row.block);
      const offset = binding?.appendixOffset;
      if (
        binding?.appendixArtifact !== logicalPath || offset === null || offset === undefined ||
        !Number.isSafeInteger(offset) || offset < 0 || offset > body.length
      ) continue;
      const before = rows.filter((entry) => entry.shard === row.shard
        ? entry.pos <= row.pos
        : entry.timestamp < row.timestamp);
      const uncertain = rows.filter((entry) =>
        entry.shard !== row.shard && entry.timestamp === row.timestamp);
      const floor = latestMainWorkflowStageRunFloorFromRows(before, "code-generation", unitMajor);
      if (
        floor !== authority.runFloor ||
        latestMainWorkflowStageRunFloorFromRows([...before, ...uncertain], "code-generation", unitMajor) !== floor
      ) continue;
      // Offsets are bytes, and may precede or follow the canonical separator
      // boundary (including an original plan that had no final newline).
      const gap = decoder.decode(body.subarray(Math.min(offset, terminal), Math.max(offset, terminal)));
      const appendix = decoder.decode(body.subarray(offset));
      if (!/^\s*$/.test(gap) || !/^\s*## Review[ \t]*(?:\r\n|\r|\n|$)/.test(appendix)) continue;
      const prefix = decoder.decode(body.subarray(0, offset));
      if (
        approvalFingerprint(prefix, artifacts.instructions, artifacts.contractHash, authority) === recorded ||
        rawApprovalFingerprint(prefix, artifacts.instructions, artifacts.contractHash, authority) === recorded
      ) return recorded;
    }
    return null;
  } catch {
...
export function evaluateCodeGenerationApproval(
  ...
  try {
    ...
    const artifacts = codeGenerationApprovalArtifacts(projectDir, authority);
    if (artifacts.expectedFingerprint !== artifacts.recordedFingerprint) {
      const reviewed = recordedApprovalBeforeReview(projectDir, authority, artifacts);
      if (reviewed !== null) artifacts.expectedFingerprint = reviewed;
    }
