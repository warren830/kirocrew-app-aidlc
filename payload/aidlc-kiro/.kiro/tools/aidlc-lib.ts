...
export function existingReviewAppendixOffset(body: Buffer): number | null {
  let text: string;
  try {
  ...
  // Derive the temporary marker from the original bytes. A heading assembled
  // with HTML entities must not impersonate a predictable internal marker.
  let marker = `AIDLCREVIEWAPPENDIXBOUNDARY${createHash("sha256").update(body).digest("hex")}X`;
...
export function latestMainWorkflowStageRunFloorFromRows(
